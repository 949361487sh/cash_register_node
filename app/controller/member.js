...
class MemberController extends BaseController {
    ...
    async getMemberList() {
        ...
        // 库存搜索
        const { state, name, tel, startTime, endTime, pageSize, pageNo } = ctx.request.body || {}
        let isState = `state = '${state}' and`
        if (state == '2') {
            isState = ''
        }
        let sql = `select * from member where ${isState} memberName like '%${name}%' and tel LIKE '%${tel}%' and createTime >= '${startTime} 00:00:00' and createTime <= '${endTime} 23:59:59' ORDER BY id DESC LIMIT ${(pageNo - 1) * 10},${pageSize}`
        let getCont = `select * from member where ${isState} memberName like '%${name}%' and tel LIKE '%${tel}%' and createTime >= '${startTime} 00:00:00' and createTime <= '${endTime} 23:59:59'`
    ...
    async addMember() {
        ...
        try {
            ...
            await this.app.mysql.insert('member', { memberName, tel, address, sex, birthday, createTime: updateTime, updateTime: updateTime, integral: 0 });
            this.message('会员注册成功')
        } catch (error) {
            this.error('会员注册失败!')
        }
